# Nidara — Working as a user's agent: where a change belongs, and contributing it back

Nidara is **AI-native**: it ships this skill so that any user running Claude Code
...
If you are an agent helping someone with their **installed** Nidara, read this
...
layer** (`~/.config/nidara/` — see `references/dev-workflow.md` for the ownership
...
gh pr list --repo nidara-project/nidara-desktop --state open \
...
gh pr list --repo nidara-project/nidara-desktop --state merged \
...
- **A match is already merged but unreleased** → the fix exists; advise `nidara-update`
...
4. **Ask the user.** Summarize the change, that it's a general improvement, and that you'd
   like to open a PR to `github.com/nidara-project/nidara-desktop` under their GitHub account.
...
5. **Branch, fork, PR.** The user almost certainly cloned the upstream repo directly and has
   ...
   **Every commit must be DCO-signed (`git commit -s`)** — the `Signed-off-by` is the *human
   ...
   gh pr create --repo nidara-project/nidara-desktop \
   ...
   **After opening, an automated triage comment appears on the PR** (deterministic — no AI
   ...
   - `needs-evidence` → the template is incomplete. Fix it by **editing the PR description**
     (`gh pr edit <n> --repo nidara-project/nidara-desktop --body-file <fixed body>`), completing
