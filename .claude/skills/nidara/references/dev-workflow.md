...
Arch-only provisioning.

⚠️ **It installs a DEVELOPMENT copy, and nothing else, since 2026-09-02 (#378 → step 3).** The
system path is gone: a bare `./install.sh` prints the package commands and exits 1. Installing the
desktop is `pacman -S nidara-desktop` from `[nidara]`, which is now first-class in the README —
one way in, and pacman owns the files. What went with the system path: the release-tag jump, the
`[nidara]` registration and key import (nothing a dev install needs comes from that repo — all 74
derived dependencies are core/extra, checked), the 62-line package branch of §4, `DEV_LIKE`, and
`nidara-update`'s STABLE path, which used to migrate a script-era install by cloning a release and
running this script on it. Migrating IS installing the package now, so that branch prints the
commands rather than being a second implementation of them.

⚠️ **It is also INTERACTIVE, and declining aborts it** (#378). Step 1 needs the system
up to date before it can install anything — Arch does not support partial upgrades — so it runs
`sudo pacman -Syu` and shows you the transaction instead of the `--noconfirm` it used to pass. The
script runs on a machine the project does not own (`nidara-iso`'s `PRODUCT.md`, "What does NOT
change"), and one unattended run of it upgraded 44 packages and both kernels on a maintainer's
desktop. Declining stops the install with nothing written, rather than continuing into the
partial-upgrade state the `-Syu` exists to prevent. **Do not wrap this in anything that expects it
to be silent** — and nothing does today: its only non-human caller is `nidara-update`'s dev
path, which runs with a user at a terminal.

Two modes:

- ~~**`--system` (default)**~~ — **retired 2026-09-02 (#378 → step 3).** It installed the
  `nidara-desktop` package, jumping to the newest release tag first and registering `[nidara]` on
  the way. All of that is what `pacman -S nidara-desktop` does natively, which is why the path went
  rather than being kept as a wrapper around it.

- **`--dev`:** installs system binaries **and** writes `~/.config/nidara/.dev` pointing at the source tree. The UI launcher will then run `scripts/run.sh app.ts` from source instead of running the bundle. Registers the developer's own clone in `.source` (no managed copy). Dev installs keep the **direct-copy** flow (§3 build + §4 copies into /usr) — and **remove the desktop package first if present** (BOTH names — a machine that has not taken the renaming upgrade still carries `nidara`), so a later `pacman -Syu` can't clobber the dev copies (this also makes the dev↔system toggle safe: going back to system reinstalls the package fresh). Iterating on the UI runs from source, so Super+Shift+R reload suffices; re-run `install.sh --dev` only when you touch system files (`bin/`, `config/`, native C libs, portals, `.desktop`) or the install flow.
- **`nidara-setup`** (`/usr/bin/nidara-setup`): the idempotent first-time/system setup that *packaging can't do* — GI env in `/etc/environment`, per-user config seeding under `~/.config/nidara` plus a default `~/.config/kitty/` (never overwrites), uwsm env files + NVIDIA autodetect, greetd/DM setup (only when recognizably ours or no DM at all), service enablement (pipewire/wireplumber user units, power-profiles-daemon, bluetooth). System-environment detection (keyboard layout / timezone / locale) lives here too, next to its consumers. Reads payloads from `/usr/share/nidara/{defaults,config}` — shipped identically by the nidara package and by install.sh §5, so it behaves the same after `pacman -S nidara-desktop` and after a script install. Called by install.sh §5 and by `nidara-update`'s package path; a pacman-only user runs it once by hand: `sudo pacman -S nidara-desktop && nidara-setup`. (The old `--dev-repo` flag is gone — it symlinked hypridle.conf into the dev repo, but Settings → Power writes that file now, so UI changes were dirtying the repo working tree; it's seeded as a per-user copy like everything else, with a migration that preserves the old link's content. Idle-suspend default: the hypridle.conf seed gets a 30-min suspend listener appended ONLY when a battery is present — desktops default to never; Settings → Power overrides either way.) **`--user` mode = the first-login bootstrap** (issue #23): runs ONLY the per-user steps (config seeding incl. `.mcp.json`, uwsm env + NVIDIA autodetect, user services) and never touches sudo. `bin/nidara` (the session entry) invokes it at session start whenever `~/.config/nidara` is missing — so a user created AFTER install time (Settings → Users, `useradd`, archinstall, Calamares) gets seeded on first login, before Hyprland loads its config, and a deleted config dir self-heals. Fail-soft by design (`|| true`, log at `${XDG_RUNTIME_DIR:-/tmp}/nidara-first-login.log`): a seeding hiccup must never block the session. GTK/dconf theming for that new user then converges at shell boot (ThemeManager reconciles appearance.json → settings.ini + gsettings).
...
- A file glob for "the package that was just built" must be **anchored**: `nidara-*` matches
  ...
  name to the version that follows it.

⚠️ **The desktop package is `nidara-desktop`, renamed from `nidara` on 2026-08-30.** The bare name
was the PRODUCT's, and `pacman -Qi nidara` was answering "what version of Nidara do you have" with
this component's number — the last place still doing that once `/etc/os-release` lost its version.
The new package carries `provides`/`replaces`/`conflicts` on the old name, so existing installs
migrate on one `pacman -Syu` (verified against a local repo: `removing nidara... installing
nidara-desktop...`) and `pacman -S nidara` still resolves. **Two consequences worth holding:**
anything that QUERIES by name must ask for both while old installs exist (`pacman -Qq nidara`
returns "not installed" on a renamed machine — it does not error, it just quietly says no), and the
bare `nidara` is deliberately free for a future metapackage meaning the whole product, which must
NOT be created while `replaces=(nidara)` is still in play.
- `install=`/`backup=` belong inside `package_nidara-desktop()`; at the top level they would apply to
  both packages.
...
`lib/live-medium.ts` names the disk the session booted from and `listDisks()`/`listPartitions()`
drop it. Before 2026-09-22 the USB stick was offered like any other disk (labelled "removable",
which is not a warning), and erasing it repartitions the device the live session is reading from.

⚠️ **The harness boots the ISO as a CD-ROM, which lsblk calls `rom`** — already dropped by the
`type === "disk"` filter — so the case only exists on real hardware, or in a VM given
`-device usb-storage`. That boot was made once, and it paid for itself twice:

- **archiso copies the image to RAM and UNMOUNTS the medium** (`/run/archiso/copytoram`, a tmpfs).
  A mount-based check returns "" on exactly the machine it protects. The second answer is the
  kernel command line: every entry in the ISO's loader config carries
  `archisosearchuuid=%ARCHISO_UUID%`, and that value is the iso9660 volume id `blkid` reports on
  the stick and on its first partition. `archisolabel=` is the same question spelled the other way.
- **`/dev/loop0` is mounted under `/run/archiso` and lsblk lists it FIRST**, so "the first node
  carrying an archiso mount" answers the squashfs loop — a path no disk in the list has, so
  nothing was excluded and the stick stayed on the page with the whole mechanism working. Only a
  `type: "disk"` node can be the answer.

Detection is deliberately NOT `rm`-based: an internal port can report a stick as fixed, and a
removable disk is a perfectly good target.
...
`const w: AtomicWidget = {...}`. **`references/writing-a-widget.md` is the full page** —
the vocabulary, the optional fields, and the boundary CI enforces; copy `calculator.ts`
as a minimal template. Then:
...
5. **Background**: implements `org.freedesktop.impl.portal.Background` (`GetAppState`, `NotifyBackground`, `EnableAutostart`) — manages background execution and autostart entries (`~/.config/autostart/`) for sandboxed Flatpaks.
...
environment variable that only hurts CHILD processes, a `hold()` whose absence
looks like a clean exit. `scripts/dev/host-probe.ts` states each of those out loud.
It needs a **live graphical Hyprland session** — it opens real windows and asks the
...
⚠️ Two silent traps it exists to catch, both found by running it:
`gi://GdkWayland?version=4.0` must be imported or GJS sees the surface as
`unknown_GdkWaylandToplevel` **with no methods** (a `typeof` guard then makes the
whole per-window app-id a no-op that logs nothing); and the override must be
applied at `map`, not `realize`, because GTK stamps the process-wide id on the
`xdg_toplevel` it creates during the first `present()`.
...
- **`/usr/share/uwsm/env-hyprland.d/nidara-browser` (from `config/uwsm/`) unsets a SYSTEM-WIDE `$BROWSER`** (2026-09-13). EndeavourOS ships `BROWSER=firefox` in `/etc/environment`, and programs that read the variable before the freedesktop associations (Python's `webbrowser`, most CLIs, "open your browser" login flows) ignored Settings → Default Apps — every freedesktop resolver said Chrome while Python opened Firefox. 🔑 **An `unset` in `bin/nidara` does NOT work**: uwsm merges the systemd user manager's environment (which already has pam_env's value) OVER the launcher's, so it comes straight back. A variable that disappears while uwsm sources its env files is one uwsm itself removes from the systemd manager (`unset_varnames` in its `prepare_env`) — that is what reaches D-Bus activation and apps. uwsm's load order is: shell profile → `/usr/share/uwsm` → `/etc/xdg/uwsm` → `~/.config/uwsm` (each `env`, `env.d/`, `env-<desktop>`, `env-<desktop>.d/`), so a user's own `export BROWSER=…` in `~/.config/uwsm/env` still wins, while one in `~/.profile` does not (it runs before). Verified by running uwsm's real `/usr/lib/uwsm/prepare-env.sh` on fake dirs: without the fragment `BROWSER=firefox`, with it unset, with it plus a user env `BROWSER=chromium`. ⚠️ Parse its output with a NUL-safe reader: `$(…)` drops the NUL separators and every case reads "unset" — the control caught that.
