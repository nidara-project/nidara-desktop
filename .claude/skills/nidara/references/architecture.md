...
| `NotifConfig.ts` | 60 | Popup auto-dismiss timeout (`notif-config.json`). Deliberately holds NO copy of Do Not Disturb — that is the daemon's own live flag, see `NotifService.ts`. |
...
| `NotifService.ts` | ~100 | **Stateless facade** over the notification daemon (`gi://AstalNotifd` today), same pattern as AudioService/BluetoothService. `notifications()` (a SNAPSHOT — the clear-all cascade holds its list while rows fly out), `getNotification(id)`, `isCritical(n)`, the Do Not Disturb trio (`dontDisturb`/`setDontDisturb`/`toggleDontDisturb`) and `watchDnd`/`watchNotified`/`watchResolved`. It was the last domain with NO facade at all: **sixteen** bare `AstalNotifd.get_default()` calls across the NC, the popups, the bar's bell, the focus tile, Settings → Notifications and `config-entries.ts`, each re-deriving the same three questions. 🔑 `watchDnd` listens on `notify::dont-disturb`; the focus tile used to listen on bare `notify` — EVERY property of the daemon — to watch one boolean. DnD has FOUR faces (CC tile, Settings row, agent key `notifications.doNotDisturb`, bar widget) and they are all this one bit, which is why the daemon's own persistence is the only copy of it. Never imports Gtk. |
