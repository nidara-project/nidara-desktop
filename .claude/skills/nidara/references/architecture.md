...
| `ThemeManager.ts` | ~800 | Appearance from its two homes (`org.gnome.desktop.interface` + `org.nidara.appearance`, followed live through `changed::`) — what ANY process needs to look like the desktop: the state, setters that only WRITE keys, and this process's own CSS providers (main/font/tokens), `Gtk.Settings` theme + prefer-dark, font sync, `style.css` hot-reload in dev. ⚠️ **No file write, subprocess or compositor call belongs here** (#571): a second process would do it a second time — those live in `AppearanceSync.ts`. **Font gotcha:** the interface font is delegated to the GNOME `font-name` gsetting, and that value is a PANGO font string — parse it with `Pango.FontDescription.from_string`, never a regex (a `/^(.*?) (\d+)$/` silently resolved style words, fractional sizes and variation axes to `sans-serif`; a dev machine rendered Noto Sans for months, found 2026-08-11). Sizes are POINTS (`fontToPoints`, see design-system.md) so GTK applies `text-scaling-factor` itself. |
