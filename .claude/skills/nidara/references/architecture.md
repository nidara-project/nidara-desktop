...
- **`surfaces/`** — whole TSX surfaces that consume `core/` state. Each surface is a function that takes a `Gdk.Monitor` and returns a `Gtk.Widget`:
  ...
  - `island/IslandWindow.ts` — the island's EXPANDED modes are the **second
    ...
    **OVERLAY** level (one above the bar's TOP). The compact capsule stays in the
    ...
    reserve. The BAR's surface is not (a non-auto-hiding side dock's exclusive
    zone insets it), which is why `sourceOffset` exists — measured from the bar's
    actual width, not recomputed from dock constants. Mapped only while a mode is
    open, and mapped **one frame before** the morph runs: the morph reads the
    capsule's bounds through this window's root, and an unmapped root yields no
    source rect, silently demoting the capsule morph to the centered fallback pop.
    Consequences you inherit when touching it (cross-window bridge, two input
    regions, keyboard-grab collision, CSS scoped to BOTH windows, layer-order
    re-assertion) are listed in `state-and-ipc.md` under "Overlay placement".
