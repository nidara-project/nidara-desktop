...
| `configFile.ts` | ~480 | Two stores behind ONE interface: `defineSettings(name, DEFAULTS, validators?, {computed?})` — a GSettings schema `org.nidara.<name>` (#573, the home every setting is moving to) — and `defineConfig(file, DEFAULTS, validators?)`, the JSON file store the rest have not left yet. Both: per-key validation, equality guard, per-key subscription. `loadKnown` is the JSON load step alone. See "Where a setting lives" below. |
...
went from 66 lines of code to 32 that way. On it today: `GamingManager` and `AgentConfig` — both
still JSON because something OUTSIDE the shell reads the file (`hyprland.lua` reads `gaming.json`;
six `bin/` helpers and the agent-loop CI test read `ai.json`), and they move to `defineSettings`
with those readers. `NotifConfig`, `barState`, `NightLightManager`, `RecordingConfig`,
`WorkspaceModes` and the dock's `state.ts` moved to `defineSettings` on 2026-09-14 — see the next
section. What is left off both is off them on purpose (see the exceptions below).
...
interface as `defineConfig`, so a module moves by changing one line. It is phase 0 of splitting the
