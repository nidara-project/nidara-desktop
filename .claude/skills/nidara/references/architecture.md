...
- **`surfaces/`** — whole TSX surfaces that consume `core/` state. Each surface is a function that takes a `Gdk.Monitor` and returns a `Gtk.Widget`:
  - `bar/`, `dock/`, `control-center/`, `app-grid/`, `island/` (the Activity Island: the bar-center capsule as a multi-purpose morphing surface. COMPACT state = a `Gtk.Stack` (crossfade + `interpolate_size`, so the pill's width animates) whose pages MUTATE by activity: workspace dots (neutral; absorbed the old `bar/Workspaces.tsx`) ↔ media compact (`PlayerIsland.tsx` `PlayerCompact`: mini art + title + 10fps Cairo EQ, EQ timer runs only while playing AND mapped). EXPANDED modes morph out of the capsule via one MorphRevealer per mode, driven by `status.island_mode` — `overview` (keyboard nav) and `player` (the shared media detail panel from `widgets/media.ts`, no keyboard grab); the native agent is the planned next one. Policy lives in `ActivityIsland.tsx`'s activity controller: playing media mutates the compact (ambient — auto-EXPAND is reserved for high-priority events), pause holds the media compact for a 12s grace (no dots↔player flicker between tracks; held while the expanded player is open), player leaving the bus reverts instantly and closes the expanded player. Capsule click expands whatever the compact currently shows; Super+W always reaches the overview), `overview/`, `prism/`
