...
- **`surfaces/`** — whole TSX surfaces that consume `core/` state. Each surface is a function that takes a `Gdk.Monitor` and returns a `Gtk.Widget`:
  - `bar/`, `dock/`, `control-center/`, `app-grid/`, `island/` (the Activity Island: the bar-center capsule as a multi-purpose morphing surface — compact state = workspace dots (absorbed the old `bar/Workspaces.tsx`), expanded MODES morph out of it via one MorphRevealer per mode, driven by `status.island_mode`; the overview is the first mode, player/agent are the planned next ones), `overview/`, `prism/`
