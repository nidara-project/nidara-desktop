...
| Astal system services | AstalHyprland, AstalWp, AstalMpris, AstalNotifd, AstalBluetooth, AstalTray, AstalApps, AstalAuth. **Dropped, do not reintroduce** (all three on 2026-08-17): AstalNetwork — `core/NetworkService.ts` sits on `libnm` directly, tech-debt #71; AstalGreet — `ui/greeter/lib/greetd.ts` speaks greetd's socket itself, it already owned the logic and was renting only the wire format; AstalBattery — `core/BatteryService.ts` sits on `UPowerGlib`, and the move found a dead `charged` branch |
