...
The Nidara icon spec (#587, `ui/shell/assets/icons/SPEC.md`) promises one meaning per name, because
...
one by adding a name (drawing + ICON_NAMES + SPEC row, and the generator's `aliases.csv`), and bump the
spec version.
...
been shipped — `ndIcon` returns null and those two sidebar rows draw no icon. And nine shipped drawings
differ from the Lucide drawing the `nidara-symbolic` theme builds under the same name (`nd-battery`,
`nd-power-profile-*`, `nd-network-wired`, `nd-user-trash`, `nd-dialog-password`,
`nd-audio-volume-muted`, `nd-network-vpn-disconnected`): hand edits or an older Lucide. Harmless while
nobody picks that theme as the interface theme — it is left off the list — but a spec theme built from
the generator would not be pixel-identical to the default.
