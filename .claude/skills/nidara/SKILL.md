...
| `typecheck` | `tsc --noEmit` over all four bundles (installer, greeter and lock in full since 2026-09-22 — their one shim is `ui/lib/gi-cairo.d.ts`, which must be FIRST in `include` or it is silently ignored), against a compressed `@girs/` snapshot pulled from the repo's `ci-assets` release (`@girs/` itself stays git-ignored, ≈58 MB generated; a maintainer refreshes the snapshot when it goes stale) |
