...
10. **No hardcoded colors. No emoji as iconography.** Resolve against `--nidara-*` tokens; use SVGs in `assets/nidara/assets/scalable/` or the `nd-*-symbolic` icon set — interface icons are asked for ONLY by `nd-` names of the Nidara icon spec (`ui/shell/assets/icons/SPEC.md`), never by freedesktop names; those are for APP icons.
