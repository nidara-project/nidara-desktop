...
| `typecheck` | `tsc --noEmit` over the shell AND the installer (the installer in full since 2026-09-22 — its `gi.d.ts` shims only `gi://cairo`; greeter/lock get a names-only pass), against a compressed `@girs/` snapshot pulled from the repo's `ci-assets` release (`@girs/` itself stays git-ignored, ≈58 MB generated; a maintainer refreshes the snapshot when it goes stale) |
