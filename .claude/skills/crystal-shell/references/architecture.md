...
| `AgentConfig.ts` | ~55 | Governance of the agent-facing surface (`ai.json`): `allowConfigWrite` gates `setConfig` writes. Toggled from Settings → AI. It is a **consent layer over the official door, not a security boundary** (any local process can still edit config files directly) — keep that framing in docs/UI copy. Reading state is never gated (doctor/diagnostics depend on it). |
