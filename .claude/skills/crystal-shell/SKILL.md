...
crystal-click <mode> <app> …            # computer-use synthetic POINTER: app/at = left-click a control's centre / window-relative point; rclick-app/rclick-at = right-click; scroll-app/scroll-at = wheel scroll (signed dx/dy notches). crystal-input.c (zwlr_virtual_pointer, compiled by install.sh) is the injector. Focus-verified; same gate. For canvas/no-a11y/list-items/context-menus/scrolling
...
crystal-shell-mcp                        # all of the above as MCP tools over stdio (incl. query_app → crystal-a11y, do_app_action → crystal-act, type_text/press_key → crystal-type, focus_window → ags request focusWindow, click_app/click_at (left + button:"right") / scroll_app/scroll_at → crystal-click; .mcp.json: repo root for dev; installer-managed copy in ~/.config/crystal-shell/ for users)
```
