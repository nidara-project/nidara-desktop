...
crystal-click app|at <app> …             # computer-use synthetic POINTER (left-click): `app <node>` clicks a control's centre, `at <x> <y>` a window-relative point; crystal-input.c (zwlr_virtual_pointer, compiled by install.sh) is the injector. Focus-verified; same gate. For canvas/no-a11y/list-items
...
crystal-shell-mcp                        # all of the above as MCP tools over stdio (incl. query_app → crystal-a11y, do_app_action → crystal-act, type_text/press_key → crystal-type, focus_window → ags request focusWindow, click_app/click_at → crystal-click; .mcp.json: repo root for dev; installer-managed copy in ~/.config/crystal-shell/ for users)
```
