...
crystal-shell-mcp                        # all of the above as MCP tools over stdio (incl. list_windows/list_workspaces (reads), focus_window (ungated WM op), query_app → crystal-a11y, do_app_action → crystal-act, type_text/press_key → crystal-type, click_app/click_at (left + button:"right") / scroll_app/scroll_at / drag_at → crystal-click; WM action verbs via run_action; .mcp.json: repo root for dev; installer-managed copy in ~/.config/crystal-shell/ for users)
```
