...
import { hypr, dragBus, mouseBus, pointerBus, savePinned, pinnedState, dockSettings, onDockSettingsChanged, menuState, onMenuCountChanged, dockSideState, onPinnedChanged } from "./state"
...
export default function DockHorizontal(gdkmonitor: any) {
    ...
    const onReorder = (sourceId: string) => {
        ...
        if (finalIdx === -1) {
            console.warn(`[DockH] onReorder: previewIdx was -1 for ${nsid}. Re-calculating...`)
        ...
        if (finalIdx === currentTotalItems - 1) {
            onUnpin(draggingId); return
        }
        if (finalIdx === 1) {
            onPin(draggingId); return
        }
    ...
    const initialHideTarget = DOCK_CONSTANTS.PILL_HEIGHT + dockSettings.screenGap + 4
    ...
    const setRevealed = (reveal: boolean) => {
        ...
        isRevealed = reveal
        slideTarget = reveal ? 0 : (DOCK_CONSTANTS.PILL_HEIGHT + dockSettings.screenGap + 4)
    ...
    const runUnifiedTick = (seedFrame = false) => {
        ...
        tickId = bar.add_tick_callback((_, clock) => {
            ...
            // Step 2: Apply per-icon layout (horizontal only)
            let totalBarWidth = 0
            let currentFloatX = 0
            let lastRoundedX = 0
            ...
            orderedIds.forEach((id) => {
                ...
                if (state.isSeparator) {
                    ...
                    if (gtkRevSep && gtkRevSep !== (revealer as any)) {
                        ...
                        if (gtkRevSep.height_request !== DOCK_CONSTANTS.PILL_HEIGHT) gtkRevSep.height_request = DOCK_CONSTANTS.PILL_HEIGHT
                    }
                    ...
                    if (gtkRev && gtkRev !== (revealer as any)) {
                        if (gtkRev.width_request !== slotW) gtkRev.width_request = slotW
    ...
    const update = (skipTargets = false) => {
        ...
        try {
            ...
            const axisSize = gdkmonitor.get_geometry().width
            ...
            console.error("[DockH] Update error:", e)
            return bar
        } finally {
            updateLock = false
            if (needsUpdate) {
    ...
    if (layerInit) {
        try {
            ...
            const exclusiveZone = dockSettings.autoHide ? 0 : DOCK_CONSTANTS.EXCLUSIVE_ZONE
