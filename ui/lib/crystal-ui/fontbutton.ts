...
import { CrystalButton } from "./button"

export interface CrystalFontButtonOpts {
...
/**
 * CrystalFontButton — pill button showing the current font (previewed in that
 * font) that opens a Gtk.FontDialog. Replaces the default Gtk.FontButton so the
 * trigger matches the Crystal look. The chooser dialog itself is GTK's (not
 * restyleable), but it opens parented/modal over its window.
 */
export function CrystalFontButton(opts: CrystalFontButtonOpts): Gtk.Button {
    const btn = CrystalButton({ variant: "secondary", pill: true, valign: Gtk.Align.CENTER })
