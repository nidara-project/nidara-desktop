...
import { CrystalSplitView, type CrystalSplitViewResult } from "./split-view"

export interface CrystalWindowOpts {
    app: any
    title: string
    /** Sidebar navigation widget — e.g. CrystalSidebar(...).widget. */
    sidebar: Gtk.Widget
...
export interface CrystalWindowResult {
    window: Gtk.Window
    /** Toggle visibility (presents on show). */
    toggle: () => void
    splitView: CrystalSplitViewResult
...
export function CrystalWindow(opts: CrystalWindowOpts): CrystalWindowResult {
    ...
    const sidebarScroll = new Gtk.ScrolledWindow({
        ...
        css_classes: ["crystal-window-sidebar-scroll"],
        vexpand: true,
    })
    ...
    // ── Glass container ───────────────────────────────────────────────────────
    const mainContainer = new Gtk.Box({ css_classes: ["crystal-window-glass"] })
    mainContainer.set_name("crystal-window-glass")
