...
/**
 * Body of ~/.config/nidara/nidara-workspaces.lua.
 *
 * Persists default workspace mode and per-workspace overrides so Hyprland's
 * Lua runtime can read them at login or reload via safe_require("nidara-workspaces")
 * without needing an ad-hoc JSON parser.
 */
export function luaWorkspaceModesBlock(
    defaultMode: string,
    modes: Record<string | number, string> = {},
): string {
    const lines = [
        "-- NIDARA WORKSPACE MODES",
        "-- Auto-generated by Nidara Shell. Do not edit manually.",
        "",
        "NIDARA_WS_MODES = {",
        `    default = ${luaLiteral(defaultMode)},`,
    ]
    const keys = Object.keys(modes).sort((a, b) => Number(a) - Number(b))
    for (const k of keys) {
        const num = Number(k)
        const keyExpr = Number.isInteger(num) && num > 0 ? `[${num}]` : luaLiteral(k)
        lines.push(`    ${keyExpr} = ${luaLiteral(modes[k])},`)
    }
    lines.push("}")
    return lines.join("\n") + "\n"
}

/** The four game-mode values the compositor needs, in the field names the shell's
 *  store uses. */
export interface GamingLuaValues {
...
/**
 * `NIDARA_GAMING = { … }` — the one statement that hands game mode's settings to
 * the compositor (#573). It is the body of ~/.config/nidara/nidara-gaming.lua
 * (required at login, so a game opened before the shell is up still sees the
 * user's choice) AND the chunk pushed live with `hyprctl eval` when a value
 * changes: an assignment EXECUTES under eval, unlike a dispatcher, which eval
 * only constructs.
 *
 * `readGamingCfg()` in hyprland.lua reads these field names; keep both in step.
 */
export function luaGamingBlock(v: GamingLuaValues): string {
    return [
        "-- NIDARA GAME MODE",
        "-- Auto-generated by Nidara Shell from org.nidara.gaming. Do not edit manually.",
        "",
        "NIDARA_GAMING = {",
        `    wallpaperMode = ${luaLiteral(v.wallpaperMode)},`,
        `    customWallpaper = ${luaLiteral(v.customWallpaper)},`,
        `    transition = ${luaLiteral(v.transition)},`,
        `    performanceProfile = ${luaLiteral(v.performanceProfile)},`,
        "}",
    ].join("\n") + "\n"
}
