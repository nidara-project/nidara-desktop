export default {
    "settings.input.title": "Devices",

    "settings.input.mouse.group": "Mouse",
    "settings.input.mouse.speed": "Pointer speed",
    "settings.input.mouse.speed.desc": "Adjust tracking sensitivity",
    "settings.input.mouse.accel": "Acceleration profile",
    "settings.input.mouse.accel.desc": "Pointer acceleration behavior",
    "settings.input.mouse.natural": "Natural scrolling",
    "settings.input.mouse.natural.desc": "Scroll in the direction of content movement",

    "settings.input.touchpad.group": "Touchpad",
    "settings.input.touchpad.natural": "Natural scrolling",
    "settings.input.touchpad.natural.desc": "Move content with fingers",
    "settings.input.touchpad.tap": "Tap to click",
    "settings.input.touchpad.tap.desc": "Tap touchpad to click",

    "settings.input.keyboard.group": "Keyboard",
    "settings.input.keyboard.numlock": "NumLock on boot",
    "settings.input.keyboard.numlock.desc": "Enable numeric keypad by default",
    "settings.input.keyboard.layout": "Keyboard layout",
    "settings.input.keyboard.layout.desc": "What this desktop types with. The text console keeps the layout the installer set",
    "settings.input.keyboard.repeat-delay": "Repeat delay",
    "settings.input.keyboard.repeat-delay.desc": "Time before a held key starts repeating",
    "settings.input.keyboard.repeat-rate": "Repeat rate",
    "settings.input.keyboard.repeat-rate.desc": "How fast a held key repeats",

    "settings.region.title": "Language & region",
    "settings.region.preview": "Live preview",

    "settings.region.time.group": "Time",
    "settings.region.time.format": "Time format",
    "settings.region.time.format.desc": "How time is displayed in the bar",
    "settings.region.time.24h": "24 hours",
    "settings.region.time.12h": "12 hours (AM/PM)",
    "settings.region.time.seconds": "Show seconds",
    "settings.region.time.seconds.desc": "Include seconds in the bar clock",

    "settings.region.date.group": "Date",
    "settings.region.date.format": "Date format",
    "settings.region.date.format.desc": "How date is displayed in the bar",
    "settings.region.date.none": "Time only — no date",
    "settings.region.date.short": "Short — Mon, Apr 6",
    "settings.region.date.short-year": "Short with year — Mon, Apr 6 2026",
    "settings.region.date.long": "Long — Monday, April 6",
    "settings.region.date.numeric": "Numeric — 04/06/2026",
    "settings.region.date.iso": "ISO 8601 — 2026-04-06",

    "settings.region.tz.group": "Timezone",
    "settings.region.tz.active": "Active timezone",
    "settings.region.tz.active.desc": "System timezone",
    "settings.region.tz.change": "Change timezone",
    "settings.region.tz.change.desc": "Type or select an IANA timezone",
    "settings.region.tz.apply": "Apply",
    "settings.region.tz.placeholder": "Ex: Europe/London",

    "settings.region.locale.group": "Language & formats",
    "settings.region.locale.lang": "Language (locale)",
    "settings.region.locale.lang.desc": "System-wide, not just your account. Takes effect on the next login.",
    "settings.region.locale.regional": "Regional format",
    "settings.region.locale.regional.desc": "Dates, numbers, currency and units. Your account only, from the next login.",
    "settings.region.locale.regional.same": "Same as language",


    // Default Apps
    "settings.defaultapps.title": "Default apps",
    "settings.defaultapps.subtitle": "Choose which app opens each file type",
    "settings.defaultapps.group.web": "Web & communication",
    "settings.defaultapps.group.media": "Files & media",
    "settings.defaultapps.group.text": "Text & productivity",
    "settings.defaultapps.browser":        "Web browser",
    "settings.defaultapps.browser.desc":   "Opens links and web pages",
    "settings.defaultapps.email":          "Email client",
    "settings.defaultapps.email.desc":     "Opens mailto: links",
    "settings.defaultapps.files":          "File manager",
    "settings.defaultapps.files.desc":     "Opens folders",
    "settings.defaultapps.images":         "Image viewer",
    "settings.defaultapps.images.desc":    "Opens photos and images",
    "settings.defaultapps.video":          "Video player",
    "settings.defaultapps.video.desc":     "Opens video files",
    "settings.defaultapps.music":          "Music player",
    "settings.defaultapps.music.desc":     "Opens audio files",
    "settings.defaultapps.pdf":            "PDF viewer",
    "settings.defaultapps.pdf.desc":       "Opens PDF documents",
    "settings.defaultapps.archive":        "Archive manager",
    "settings.defaultapps.archive.desc":   "Opens zip and archive files",
    "settings.defaultapps.editor":         "Text editor",
    "settings.defaultapps.editor.desc":    "Opens plain text files",
    "settings.defaultapps.calendar":       "Calendar",
    "settings.defaultapps.calendar.desc":  "Opens calendar files (.ics)",

    // Accessibility
    "settings.accessibility.title": "Accessibility",
    "settings.accessibility.group.vision": "Vision",
    "settings.accessibility.group.motion": "Motion",
    "settings.accessibility.text-scale":        "Text size",
    "settings.accessibility.text-scale.desc":   "Scale factor for all text",
    "settings.accessibility.cursor-size":       "Cursor size",
    "settings.accessibility.cursor-size.desc":  "Size of the mouse pointer",
    "settings.accessibility.reduce-motion":        "Reduce motion",
    "settings.accessibility.reduce-motion.desc":        "Overlays appear without their pop, and window and workspace animations are switched off.",

    // Users
    "settings.users.title": "Users",
    "settings.users.group.profile": "Profile",
    "settings.users.group.security": "Security",
    "settings.users.avatar.change":       "Change…",
    "settings.users.avatar.pick":         "Choose a profile picture",
    "settings.users.avatar.filter":       "Images",
    "settings.users.avatar.crop.title":   "Adjust photo",
    "settings.users.avatar.crop.hint":    "Drag to reposition · slide to zoom",
    "settings.users.avatar.crop.use":     "Use photo",
    "settings.users.name":                "Display name",
    "settings.users.name.desc":           "Your full name shown to other users",
    "settings.users.name.apply":          "Apply",
    "settings.users.username":            "Username",
    "settings.users.password":            "Password",
    "settings.users.password.desc":       "Opens a terminal to change your password",
    "settings.users.password.change":     "Change password…",
    "settings.users.password.done":       "Press Enter to close",
    "settings.users.group.other":         "Other users",
    "settings.users.other.empty":         "No other user accounts",
    "settings.users.other.add":           "Add user…",
    "settings.users.other.admin":         "Administrator",
    "settings.users.other.admin.tip":     "Toggle administrator (wheel group) access",
    "settings.users.other.delete":        "Delete user",
    "settings.users.other.cancel":        "Cancel",
    "settings.users.other.create":        "Create",
    "settings.users.other.fullname":      "Full name",
    "settings.users.other.fullname.placeholder": "e.g. Jane Smith",
    "settings.users.other.username":      "Username",
    "settings.users.other.username.placeholder": "e.g. jane",
    "settings.users.other.delete.confirm.title": "Delete user",
    "settings.users.other.delete.confirm.body":  "Permanently delete the account and home folder for",
    "settings.users.other.err.create":    "Failed to create user. Check permissions.",
    "settings.users.other.err.exists":    "That username already exists.",
    "settings.users.other.pw":            "Password",
    "settings.users.other.pw.placeholder": "Leave blank for locked account",
    "settings.users.other.pw2":           "Confirm password",
    "settings.users.other.pw2.placeholder": "Repeat password",
    "settings.users.other.err.pwmatch":   "Passwords do not match",
    "settings.users.other.err.pw":        "Failed to change password.",
    "settings.users.other.err.pw-set":    "User created, but the password could not be set. Try again.",
    "settings.users.other.pw.change":     "Change password",
    "settings.users.other.pw.apply":      "Apply",

    // About
    "settings.about.group.nidara": "Nidara",
    "settings.about.group.environment": "Environment",
    "settings.about.group.system": "System",
    "settings.about.title": "About",
    "settings.about.uptime.desc": "Since last boot",
    "settings.about.cpu.desc": "Processor",
    "settings.about.ram.desc": "Total installed",
    "settings.about.hyprland.desc": "Compositor version",
    "settings.about.kernel.desc": "Linux kernel version",
    "settings.about.cpu": "CPU",
    "settings.about.hyprland": "Hyprland",
    "settings.about.kernel": "Kernel",
    "settings.about.ram": "RAM",
    "settings.about.os": "Operating system",
    "settings.about.device": "Device",
    "settings.about.uptime": "Uptime",
    "settings.about.version": "Version",
    "settings.about.unavailable": "Not available",
    "settings.about.graphics": "Graphics",
    "settings.about.close": "Close",
    "settings.about.device.desc": "This computer's name",
    "settings.about.os.desc": "Installed distribution",
    "settings.about.graphics.desc": "Graphics adapter",
    "settings.about.gtk.desc": "UI toolkit",
    "settings.about.gjs.desc": "JavaScript runtime",
    "settings.about.more-info": "More information…",
    "settings.about.update": "Software update",
    "settings.about.update.up-to-date": "You're on the latest release",
    "settings.about.update.available.desc": "Update by running `nidara-update` in a terminal",

    // Appearance
    "settings.appearance.group.base-style": "Base style",
    "settings.appearance.group.theme": "Theme",
    "settings.appearance.group.wallpaper": "Wallpaper",
    "settings.appearance.group.fonts": "Fonts",
    "settings.appearance.group.night-light": "Night light",
    "settings.appearance.night-light": "Night light",
    "settings.appearance.night-light.desc": "Reduce blue light in the evening",
    "settings.appearance.night-light-temp": "Color temperature",
    "settings.appearance.night-light-temp.desc": "Lower values are warmer and easier on the eyes",
    "settings.appearance.night-light-schedule": "Activate automatically",
    "settings.appearance.night-light-schedule.desc": "Turn Night Light on and off on a schedule",
    "settings.appearance.night-light-from": "Turn on at",
    "settings.appearance.night-light-to": "Turn off at",
    "settings.appearance.group.resources": "System resources",
    "settings.appearance.browse": "Browse…",
    "settings.appearance.title": "Appearance",
    "settings.appearance.window-glass.desc": "Opacity of the Settings and About windows",
    "settings.appearance.accent.desc": "Defines the vibrant tone of the interface",
    "settings.appearance.transition.desc": "Effect when changing the wallpaper",
    "settings.appearance.image.desc": "Choose wallpaper from your files",
    "settings.appearance.gtk-theme.desc": "App theme style",
    "settings.appearance.cursor.desc": "Cursor style",
    "settings.appearance.icons.desc": "System icon pack",
    "settings.appearance.dark-mode.desc": "Enable dark appearance",
    "settings.appearance.accent": "Accent color",
    "settings.appearance.cursor": "Cursor",
    "settings.appearance.icons": "Icons",
    "settings.appearance.image": "Image",
    "settings.appearance.dark-mode": "Dark mode",
    "settings.appearance.shell-appearance": "Bar & dock appearance",
    "settings.appearance.shell-appearance.desc": "Follow the system theme, or pin the bar and dock to dark or light",
    "settings.appearance.shell-appearance.system": "Follow system",
    "settings.appearance.shell-appearance.dark": "Always dark",
    "settings.appearance.shell-appearance.light": "Always light",
    "settings.appearance.gtk-theme": "GTK theme",
    "settings.appearance.transition": "Transition",
    "settings.appearance.shell-opacity": "Shell glass",
    "settings.appearance.shell-opacity.desc": "Opacity of the bar, panels and overlays",
    "settings.appearance.dock-opacity": "Dock glass",
    "settings.appearance.dock-opacity.desc": "Opacity of the dock surface",
    "settings.appearance.glass": "Glass",
    "settings.appearance.glass.desc": "Overall opacity of the bar, dock, overlays and windows",
    "settings.appearance.advanced": "Advanced",
    "settings.appearance.bar-opacity": "Bar glass",
    "settings.appearance.bar-opacity.desc": "Opacity of the bar capsules",
    "settings.appearance.overlay-opacity": "Overlay glass",
    "settings.appearance.overlay-opacity.desc": "Opacity of the Control Center, notifications and panels",
    "settings.appearance.window-glass": "Window glass",
    "settings.appearance.dialog.wallpaper": "Select wallpaper",
    "settings.appearance.filter.images": "Images",
    "settings.appearance.interface-font": "Interface font",
    "settings.appearance.interface-font.desc": "Font used in the shell and GTK apps",
    "settings.appearance.mono-font": "Monospace font",
    "settings.appearance.mono-font.desc": "Font for terminals and code editors",

    // Wallpaper transitions
    "wallpaper.transition.random": "Random",
    "wallpaper.transition.simple": "Soft fade",
    "wallpaper.transition.fade": "Fade with curve",
    "wallpaper.transition.center": "Center circle",
    "wallpaper.transition.grow": "Expand",
    "wallpaper.transition.outer": "Contract",
    "wallpaper.transition.wipe": "Diagonal wipe",
    "wallpaper.transition.wave": "Wave",
    "wallpaper.transition.left": "From left",
    "wallpaper.transition.right": "From right",
    "wallpaper.transition.top": "From top",
    "wallpaper.transition.bottom": "From bottom",

    // Apps
    "settings.apps.choose-image": "Choose image…",
    "settings.apps.restore": "Restore",
    "settings.apps.subtitle": "Customize icons for installed apps",
    "settings.apps.section": "Apps",
    "settings.apps.title": "Installed apps",
    "settings.apps.tooltip.remove-override": "Remove override and restore theme icon",
    "settings.apps.badge.override": "override",
    "settings.apps.badge.sandboxed": "Sandboxed",
    "settings.apps.badge.unsandboxed": "Not sandboxed",
    "settings.apps.origin.flatpak": "Flatpak",
    "settings.apps.origin.system": "System package",
    "settings.apps.origin.user": "Installed in your home folder",
    "settings.apps.origin.snap": "Snap",
    "settings.apps.permissions.group": "Permissions",
    "settings.apps.permissions.footer": "This app is sandboxed: it can only use these with your permission. “Ask” shows the prompt the next time it requests one.",
    "settings.apps.permissions.camera": "Camera",
    "settings.apps.permissions.wallpaper": "Change the wallpaper",
    "settings.apps.permissions.ask": "Ask",
    "settings.apps.permissions.allow": "Allow",
    "settings.apps.permissions.deny": "Deny",
    "settings.apps.install.group": "Granted at install",
    "settings.apps.install.footer": "Given without asking when the app was installed. Changes apply the next time you open it.",
    "settings.apps.install.network": "Network",
    "settings.apps.install.sound": "Sound and microphone",
    "settings.apps.install.gpu": "Graphics acceleration",
    "settings.apps.install.home": "Home folder",
    "settings.apps.install.requested": "Requested by the app",
    "settings.apps.install.not-requested": "Not requested by the app",
    "settings.apps.install.restore": "What the app requested",
    "settings.apps.install.restore.desc": "Undo your changes to these permissions",
    "settings.apps.dialog.icon": "Icon",
    "settings.apps.dialog.select-icon": "Select icon",
    "settings.apps.entry.search": "Search app…",
    "settings.apps.filter.images": "SVG / PNG images",
    "settings.apps.detail.group.icon": "Icon",
    "settings.apps.detail.icon.desc": "Choose an SVG or PNG image, or restore the theme icon",

    // Audio
    "settings.audio.group.output": "Output devices",
    "settings.audio.group.input": "Input devices",
    "settings.audio.error.no-service": "Audio service not available",
    "settings.audio.error.no-hardware": "No sound hardware detected",
    "settings.audio.title": "Sound",
    "settings.audio.device": "Device",
    "settings.audio.default": "Default",
    "settings.audio.btn.set-default": "Set default",
    "settings.audio.group.apps": "App volume",
    "settings.audio.no-apps": "No apps playing audio",

    // Autostart
    "settings.autostart.group.entries": "Startup apps",
    "settings.autostart.subtitle": "Choose which apps launch at startup",
    "settings.autostart.add": "Add",
    "settings.autostart.add-app": "Add startup app…",
    "settings.autostart.added": "added",
    "settings.autostart.apply-note": "Changes apply the next time Hyprland starts.",
    "settings.autostart.empty": "No startup apps yet",
    "settings.autostart.group.custom": "Custom command",
    "settings.autostart.title": "Autostart",
    "settings.autostart.tooltip.remove": "Remove",
    "settings.autostart.entry.placeholder": "Command (e.g. nm-applet)",

    // Bluetooth
    "settings.bluetooth.group.search": "Search devices",
    "settings.bluetooth.group.detected": "Detected devices",
    "settings.bluetooth.group.paired": "Paired devices",
    "settings.bluetooth.search-now": "Search now",
    "settings.bluetooth.connect": "Connect",
    "settings.bluetooth.disconnect": "Disconnect",
    "settings.bluetooth.pair": "Pair",
    "settings.bluetooth.no-devices": "No devices",
    "settings.bluetooth.error.no-adapter": "Bluetooth adapter not available",
    "settings.bluetooth.title": "Bluetooth",
    "settings.bluetooth.scan.desc": "Scan for nearby Bluetooth devices for 8 seconds",
    "settings.bluetooth.enable.desc": "Turn the Bluetooth adapter on or off",
    "settings.bluetooth.enable": "Enable Bluetooth",
    "settings.bluetooth.scan": "Scan",
    "settings.bluetooth.tooltip.forget": "Forget device",
    "settings.bluetooth.pairing.confirm": "Confirm",
    "settings.bluetooth.pairing.cancel": "Cancel",
    "settings.bluetooth.pairing.close": "Close",
    "settings.bluetooth.pairing.confirm-code": "Confirm that this code is shown on the device:",
    "settings.bluetooth.pairing.enter-on-device": "Type this code on the device, then confirm there:",
    "settings.bluetooth.pairing.enter-passkey": "Enter the 6-digit code shown on the device:",
    "settings.bluetooth.pairing.enter-pin": "Enter the PIN for the device:",
    "settings.bluetooth.pairing.authorize": "This device wants to connect to this computer.",

    // Control Center
    "settings.widgets.tooltip.no-space": "Not enough space — remove a control first",
    "settings.widgets.tooltip.no-hardware": "No compatible hardware detected on this device",

    // Display
    "settings.display.error.no-monitors": "No monitors detected.",
    "settings.display.title": "Display",
    "settings.display.group.all": "All displays",
    "settings.display.scale.desc": "Display scale factor",
    "settings.display.model.desc": "Monitor identifier",
    "settings.display.resolution.desc": "Resolution and refresh rate",
    "settings.display.confirm.title": "Keep display settings?",
    "settings.display.confirm.body": "Reverting to the previous setting in %d seconds…",
    "settings.display.confirm.keep": "Keep changes",
    "settings.display.confirm.revert": "Revert",
    "settings.display.rotation.desc": "Screen orientation",
    "settings.display.vrr.desc": "Variable refresh rate (requires compatible display)",
    "settings.display.scale": "Scale",
    "settings.display.model": "Model",
    "settings.display.resolution": "Active resolution",
    "settings.display.rotation": "Rotation",
    "settings.display.vrr": "VRR / FreeSync",
    "settings.display.label.unknown": "Unknown",
    "settings.display.label.builtin": "integrated",
    "settings.display.label.monitor": "Monitor",
    "settings.display.rotation.normal": "Normal (0°)",
    "settings.display.vrr.off": "Disabled",
    "settings.display.vrr.always": "Always",
    "settings.display.vrr.fullscreen": "Fullscreen only",

    // Dock
    "settings.dock.group.behavior": "Behavior",
    "settings.dock.group.effects": "Effects",
    "settings.dock.group.geometry": "Geometry",
    "settings.dock.group.position": "Position",
    "settings.dock.opt.right": "Right",
    "settings.dock.side-autohide-note": "In side position, auto-hide is always enabled.",
    "settings.dock.opt.bottom": "Bottom",
    "settings.dock.opt.left": "Left",
    "settings.notif.title": "Notifications",
    "settings.notif.group.popups": "Popups",
    "settings.notif.timeout": "Auto-dismiss after",
    "settings.notif.timeout.desc": "How long a popup stays on screen before hiding",
    "settings.notif.dnd": "Do not disturb",
    "settings.notif.dnd.desc": "Silence notification popups. Critical alerts still come through.",

    "settings.ai.title": "AI",
    // Group footers answer the question the titles cannot: WHO does this govern?
    // Some of these permissions apply to every agent, some only to the built-in
    // Assistant, and nothing on the page said which was which.
    "settings.ai.group.access": "Desktop access",
    "settings.ai.group.access.scope": "Applies to every agent — the built-in Assistant and any client connected over MCP.",
    "settings.ai.group.other-apps": "Other apps",
    "settings.ai.group.other-apps.scope": "Applies to every agent. Unlike the permissions above, these reach outside Nidara and into whichever app you have focused.",
    "settings.ai.group.mcp": "MCP server",
    "settings.ai.group.mcp.scope": "The channel connected clients arrive through. Turning it off disconnects them and changes nothing for the built-in Assistant.",
    "settings.ai.allow-config-write": "Allow agents to change settings",
    "settings.ai.allow-config-write.desc": "Local agents and scripts can modify shell settings through the official interface. Reading state is always available.",
    "settings.ai.allow-screenshot": "Allow agents to capture the screen",
    "settings.ai.allow-screenshot.desc": "Lets agents take a screenshot to visually verify their changes (`nidara-ipc screenshot`)",
    "settings.ai.allow-window-close": "Close windows",
    "settings.ai.allow-window-close.desc": "Let the assistant close windows when you ask it to. It asks the window to close, so apps can still prompt to save.",
    "settings.ai.group.files": "Files",
    "settings.ai.group.files.scope": "The built-in Assistant only. Connected clients such as Claude Code bring their own file access, which Nidara neither provides nor governs.",
    "settings.ai.allow-file-read": "Let the Assistant read your configuration",
    "settings.ai.allow-file-read.desc": "Lets the built-in Assistant read Nidara's own settings, the shipped defaults and the shell log, so it can diagnose a problem instead of guessing. It never reads the rest of your files. What it does read is sent to your chosen model provider.",
    "settings.ai.allow-file-write": "Let the Assistant edit your configuration",
    "settings.ai.allow-file-write.desc": "Lets the built-in Assistant edit the few files no running service owns — your personal Hyprland overrides and the idle settings. Off by default: those files hold commands that run when your session starts, so editing them schedules commands. Everything it writes is plain text you can read and undo. Requires the setting above.",
    "settings.ai.allow-mcp": "Enable MCP server",
    "settings.ai.allow-mcp.desc": "Lets `nidara-mcp` serve shell tools (actions, settings, state, screenshots, diagnostics) to MCP clients like Claude Code. Takes effect immediately.",
    "settings.ai.allow-computer-use": "Allow agents to see other apps",
    "settings.ai.allow-computer-use.desc": "Lets agents read the accessibility tree of third-party apps (their on-screen controls and text) for the computer-use layer. Read-only. Off by default — this reaches outside the shell; enabling it also turns on system accessibility.",
    "settings.ai.allow-computer-control": "Allow agents to control other apps",
    "settings.ai.allow-computer-control.desc": "Lets agents act on third-party apps — invoking a control's own accessibility action (activate a button, toggle a checkbox), and, into the focused app, typing with a synthetic keyboard and clicking with a synthetic pointer. Off by default and requires the setting above. While on, a red indicator stays in the bar; click it (or press Super+Shift+Esc) to stop control instantly.",
    "settings.ai.group.surface": "Agent interface",
    "settings.ai.exposed-settings": "Exposed settings",
    "settings.ai.exposed-settings.desc": "Settings agents can discover and change — run `nidara-ipc describeConfig` to list them",
    "settings.ai.state-read": "State & diagnostics",
    "settings.ai.state-read.desc": "`dumpState` and nidara-doctor stay available regardless of the toggles above",
    "settings.ai.state-read.value": "Always available",
    "settings.ai.connect-agent": "Connect your agent",
    "settings.ai.connect-agent.desc": "Tell your AI agent to register the MCP server described in this file — it works from any MCP client",

    // Built-in Assistant — the brain (BYOK); key lives in the keyring, never in files
    "settings.ai.brain.group": "Assistant",
    "settings.ai.brain.group.scope": "Which model Nidara's own Assistant talks to, and the key it uses. Connected clients bring their own model and key.",
    // Provider names are proper nouns and stay untranslated (see Ai.tsx) — only
    // these three are strings.
    "settings.ai.brain.provider": "Provider",
    "settings.ai.brain.provider.desc": "Which service runs the built-in assistant's model. Bring your own key.",
    "settings.ai.brain.provider.off": "Off",
    "settings.ai.brain.provider.ollama": "Ollama (local)",
    "settings.ai.brain.provider.localhost": "Local server…",
    "settings.ai.brain.provider.custom": "Other API endpoint…",
    "settings.ai.brain.model": "Model",
    "settings.ai.brain.model.desc": "Model id the assistant talks to (e.g. claude-opus-4-8, or a local model name).",
    "settings.ai.brain.endpoint": "Endpoint",
    "settings.ai.brain.endpoint.desc": "Base URL of the OpenAI-compatible server (e.g. a local Ollama at http://localhost:11434/v1).",
    "settings.ai.brain.model.placeholder": "Find models, or type an id",
    "settings.ai.brain.model.fetch": "Find models",
    "settings.ai.brain.model.choose": "Choose a model…",
    "settings.ai.brain.model.fetching": "Asking the provider…",
    "settings.ai.brain.model.failed": "Couldn't read the model list: %s. You can still type the id.",
    "settings.ai.brain.model.needkey": "Save your API key first — the model list is behind it.",
    "settings.ai.brain.key": "API key",
    "settings.ai.brain.key.desc": "Stored in the system keyring, never in plain files.",
    "settings.ai.brain.key.placeholder": "Paste API key…",
    "settings.ai.brain.key.stored": "•••••••• stored in keyring",
    "settings.ai.brain.key.unavailable": "Keyring unavailable",
    "settings.ai.brain.key.save": "Save key",
    "settings.ai.brain.key.clear": "Forget",
    "settings.ai.brain.key.saved": "Key saved to the keyring.",
    "settings.ai.brain.key.replaced": "Key replaced — the previous one for this provider is gone.",
    "settings.ai.brain.key.cleared": "Key forgotten. The assistant can't reach this provider until you save a new one.",
    "settings.ai.brain.key.failed": "Couldn't write to the keyring. The key was not saved.",

    // Built-in Assistant — the visual signal while a turn runs (core/AgentGlow.ts)
    "settings.ai.group.signal": "While it works",
    "settings.ai.group.signal.scope": "What the desktop shows you while the Assistant is doing something, so you can tell its work from yours.",
    "settings.ai.assistant-glow": "Glow the window it's working in",
    "settings.ai.assistant-glow.desc": "The focused window's border lights up for as long as a turn runs, and sweeps once when the Assistant takes a window over. Needs Hyprland 0.56 or newer.",

    // Badge on the fake AI cursor (surfaces/agent-pointer/)
    "agentPointer.badge": "AI",

    "settings.bar.title": "Top bar",
    "settings.bar.group.layout": "Layout",
    "settings.bar.app-title": "Active app title",
    "settings.bar.app-title.desc": "Show the focused window name in the bar",
    "settings.bar.group.icon": "Menu button icon",
    "settings.bar.icon-preset": "Default",
    "settings.bar.icon-custom": "Custom icon",
    "settings.bar.icon-custom.desc": "Choose an SVG or PNG image, or reset to the Nidara logo",

    "settings.dock.title": "Dock",
    "settings.dock.bottom-margin.desc": "Distance to screen edge",
    "settings.dock.position.desc": "Where the dock appears on screen",
    "settings.dock.magnification.desc": "Zoom effect on hover",
    "settings.dock.autohide.desc": "Dock hides when cursor moves away",
    "settings.dock.max-size.desc": "Expansion limit when magnifying",
    "settings.dock.indicators.desc": "Dot below icons of running apps",
    "settings.dock.hide-delay.desc": "Time before dock hides",
    "settings.dock.magnification": "Magnification",
    "settings.dock.bottom-margin": "Bottom margin",
    "settings.dock.indicators": "Show indicators",
    "settings.dock.autohide": "Auto-hide",
    "settings.dock.position": "Dock position",
    "settings.dock.hide-delay": "Hide delay",
    "settings.dock.max-size": "Max size",
    "settings.dock.icon-size": "Icon size",
    "settings.dock.icon-size.desc": "Base size at rest",

    // Desktop (#513)
    "settings.desktop.title": "Desktop",
    "settings.desktop.group.default": "Default Mode",
    "settings.desktop.group.workspaces": "Per-Workspace Mode",
    "settings.desktop.default-mode": "Default window mode",
    "settings.desktop.default-mode.desc": "Layout inherited by any workspace without an explicit setting",
    "settings.desktop.default-mode.opt.floating": "Floating",
    "settings.desktop.default-mode.opt.tiling": "Tiling",
    "settings.desktop.mode.default": "Default",
    "settings.desktop.mode.floating": "Floating",
    "settings.desktop.mode.tiling": "Tiling",
    "settings.desktop.workspace-1": "Workspace 1",
    "settings.desktop.workspace-1.desc": "Window mode for workspace 1",
    "settings.desktop.workspace-1.opt.default": "Default",
    "settings.desktop.workspace-1.opt.floating": "Floating",
    "settings.desktop.workspace-1.opt.tiling": "Tiling",
    "settings.desktop.workspace-2": "Workspace 2",
    "settings.desktop.workspace-2.desc": "Window mode for workspace 2",
    "settings.desktop.workspace-2.opt.default": "Default",
    "settings.desktop.workspace-2.opt.floating": "Floating",
    "settings.desktop.workspace-2.opt.tiling": "Tiling",
    "settings.desktop.workspace-3": "Workspace 3",
    "settings.desktop.workspace-3.desc": "Window mode for workspace 3",
    "settings.desktop.workspace-3.opt.default": "Default",
    "settings.desktop.workspace-3.opt.floating": "Floating",
    "settings.desktop.workspace-3.opt.tiling": "Tiling",
    "settings.desktop.workspace-4": "Workspace 4",
    "settings.desktop.workspace-4.desc": "Window mode for workspace 4",
    "settings.desktop.workspace-4.opt.default": "Default",
    "settings.desktop.workspace-4.opt.floating": "Floating",
    "settings.desktop.workspace-4.opt.tiling": "Tiling",
    "settings.desktop.workspace-5": "Workspace 5",
    "settings.desktop.workspace-5.desc": "Window mode for workspace 5",
    "settings.desktop.workspace-5.opt.default": "Default",
    "settings.desktop.workspace-5.opt.floating": "Floating",
    "settings.desktop.workspace-5.opt.tiling": "Tiling",

    // Network
    "settings.network.group.ethernet": "Wired (Ethernet)",
    "settings.network.group.wireless": "Wireless",
    "settings.network.group.access-points": "Nearby access points",
    "settings.network.group.wi-fi": "Wi-Fi",
    "settings.network.error.no-service": "Network service not available",
    "settings.network.title": "Network",
    "settings.network.access-point.desc": "Current wireless network assignment",
    "settings.network.hw-status.desc": "Current status of the physical interface",
    "settings.network.enable-wifi.desc": "Turn Wi-Fi on or off",
    "settings.network.ip.desc": "Unique identifier on local network",
    "settings.network.no-adapter.desc": "No compatible adapter found",
    "settings.network.wireless-interface.desc": "Wireless adapter name",
    "settings.network.kernel-device.desc": "Kernel device name",
    "settings.network.connected-network.desc": "Currently connected network",
    "settings.network.speed.desc": "Maximum theoretical throughput",
    "settings.network.enable-wifi": "Enable Wi-Fi",
    "settings.network.ethernet": "Ethernet connection",
    "settings.network.ip": "IP address",
    "settings.network.ipv4": "IPv4 address",
    "settings.network.hw-status": "Hardware status",
    "settings.network.interface": "Interface",
    "settings.network.access-point": "Access point",
    "settings.network.speed": "Speed",
    "settings.network.status.connected": "Connected",
    "settings.network.status.disconnected": "Disconnected",
    "settings.network.status.disconnected-hw": "Disconnected",
    "settings.network.error.no-wifi-hw": "Wi-Fi hardware not detected",
    "settings.network.label.none": "None",
    "settings.network.ap.connect": "Connect",
    "settings.network.ap.disconnect": "Disconnect",
    "settings.network.ap.connecting": "Connecting…",
    "settings.network.ap.label.error": "Error",
    "settings.network.ap.scan": "Scan",
    "settings.network.ap.empty": "No networks found. Tap Scan to search.",
    "settings.network.ap.forget": "Forget this network",
    "settings.network.ap.details": "Network details",
    "settings.network.security.open": "Open",
    "settings.network.security.enterprise": "Enterprise (802.1X)",
    "settings.network.detail.group.info": "Network",
    "settings.network.detail.group.ipv4": "Connection",
    "settings.network.detail.security": "Security",
    "settings.network.detail.security.desc": "Authentication and encryption",
    "settings.network.detail.signal": "Signal",
    "settings.network.detail.signal.desc": "Current signal strength",
    "settings.network.detail.band": "Band",
    "settings.network.detail.band.desc": "Frequency band",
    "settings.network.detail.channel": "Channel",
    "settings.network.detail.channel.desc": "Operating channel",
    "settings.network.detail.frequency": "Frequency",
    "settings.network.detail.frequency.desc": "Center frequency",
    "settings.network.detail.bssid": "BSSID",
    "settings.network.detail.bssid.desc": "Access point hardware address",
    "settings.network.detail.max-rate": "Max rate",
    "settings.network.detail.max-rate.desc": "Maximum supported bitrate",
    "settings.network.detail.ipv4.desc": "Assigned address",
    "settings.network.detail.gateway": "Gateway",
    "settings.network.detail.gateway.desc": "Default route",
    "settings.network.detail.dns": "DNS",
    "settings.network.detail.dns.desc": "Name servers",
    "settings.network.detail.mac": "MAC address",
    "settings.network.detail.mac.desc": "This device's hardware address",
    "settings.network.detail.speed.desc": "Current link speed",
    "settings.network.ap.failed": "Couldn't connect",
    "network.prompt.heading": "“%s” needs a password",
    "network.prompt.heading.retry": "Couldn't connect to “%s”",
    "network.prompt.body": "Enter the Wi-Fi password to join this network.",
    "network.prompt.body.retry": "The password may be wrong. Check it and try again.",
    "network.prompt.placeholder": "Password",
    "network.prompt.cancel": "Cancel",
    "network.prompt.connect": "Connect",
    "settings.network.group.vpn": "VPN",
    "settings.network.vpn.no-profiles": "No VPN profiles configured",
    "settings.network.vpn.btn.connect": "Connect",
    "settings.network.vpn.btn.disconnect": "Disconnect",
    "settings.network.vpn.btn.connecting": "Connecting…",
    "widget.vpn.name": "VPN",
    "widget.vpn.sub.disconnected": "Not connected",

    // Power
    "settings.power.group.idle": "Idle & lock",
    "settings.power.group.profile": "Performance profile",
    "settings.power.profile.power-saver": "Power saver",
    "settings.power.profile.performance": "High performance",
    "settings.power.lock-note": "Lock must occur before suspend to ensure security.",
    "settings.power.profile.balanced": "Balanced",
    "settings.power.opt.never": "Never",
    "settings.power.time.min": "min",
    "settings.power.time.hour": "hour",
    "settings.power.time.hours": "hours",
    "settings.power.title": "Power",
    "settings.power.screen-off.desc": "Time inactive before turning off the screen",
    "settings.power.lock.desc": "Time inactive before locking the session",
    "settings.power.suspend.desc": "Time inactive before suspending",
    "settings.power.screen-off": "Turn off screen",
    "settings.power.lock": "Lock session",
    "settings.power.suspend": "Suspend",

    // Widgets
    "settings.widgets.show-in-bar": "Show in the bar",
    "settings.widgets.show-in-cc": "Show in the control center",
    "settings.widgets.col.bar": "Bar",
    "settings.widgets.col.cc": "Center",
    "settings.widgets.configure": "Configure",
    "settings.widgets.reorder-note": "To reorder controls in the Control Center, use the panel's Edit mode.",
    "settings.widgets.title": "Control center",
    "settings.widgets.category.system": "System",
    "settings.widgets.category.utilities": "Utilities",
    "settings.widgets.category.media": "Media",
    "cc.status.stop": "Stop",
    "cc.status.recording.label": "Screen recording",
    "cc.status.recording.detail": "Capturing the screen",
    "cc.status.ai.label": "AI control",
    "cc.status.ai.armed": "Allowed to control your apps",
    "cc.status.ai.active": "Controlling apps right now",

    // Dock item context menu
    "settings.dock.dockitem.empty-trash": "Empty trash",
    "settings.dock.dockitem.close-all": "Close all",
    "settings.dock.dockitem.quit": "Quit",
    "settings.dock.dockitem.unpin": "Unpin from dock",
    "settings.dock.dockitem.keep": "Keep in dock",

    // Prism (search)
    "prism.placeholder": "Search apps and recent files…",
    "prism.section.apps": "Applications",
    "prism.section.files": "Recent files",

    // Workspace Overview
    "workspace.mode.floating": "Floating",
    "workspace.mode.tiling": "Tiling",
    "overview.workspace": "Workspace",
    "overview.empty": "Empty",
    "overview.window": "window",
    "overview.windows": "windows",

    // Settings window chrome
    "settings.search.placeholder": "Search settings…",
    "settings.nav.back":           "Back",
    "settings.nav.forward":        "Forward",
    "settings.nav.menu":           "Menu",
    "settings.window.close":       "Close",
    "settings.window.sidebar-toggle": "Show or hide the sidebar",
    "settings.search.no-results":  "No results",
    "settings.page.load-error":    "Failed to load page",

    // App Grid
    "app-grid.search.placeholder": "Search apps…",
    "app-grid.no-results": "No results",
    "app-grid.menu.pin": "Pin to dock",

    // Dock special item names and menu
    "dock.special.launcher.name": "Applications",
    "dock.special.trash.name": "Trash",
    "dock.special.home.label": "Files",
    "dock.menu.open": "Open",
    "dock.menu.window-of": "Window of",

    // System menu
    "bar.system-menu.about": "About this computer",
    "bar.system-menu.settings": "System settings…",
    "bar.system-menu.lock": "Lock screen",
    "bar.system-menu.suspend": "Sleep",
    "bar.system-menu.logout": "Log out…",
    "bar.system-menu.restart": "Restart…",
    "bar.system-menu.shutdown": "Shut down…",
    "bar.system-menu.confirm.logout": "Log out?",
    "bar.system-menu.confirm.restart": "Restart this computer?",
    "bar.system-menu.confirm.shutdown": "Shut down this computer?",
    "bar.system-menu.confirm.cancel": "Cancel",
    "bar.system-menu.confirm.action.logout": "Log out",
    "bar.system-menu.confirm.action.restart": "Restart",
    "bar.system-menu.confirm.action.shutdown": "Shut down",

    // Window menu (AppTitle)
    "bar.window-menu.float": "Float",
    "bar.window-menu.pseudo": "Pseudo-tile",
    "bar.window-menu.fullscreen": "Fullscreen",
    "bar.window-menu.center": "Center on screen",
    "bar.window-menu.pin": "Pin to all workspaces",
    "bar.window-menu.move-to": "Move to workspace",
    "bar.window-menu.workspace-mode": "Mode for workspace",
    "bar.window-menu.workspace": "Workspace",
    "bar.window-menu.float-all": "Float all windows",
    "bar.window-menu.no-window": "No focused window",
    "bar.window-menu.group": "Group",
    "bar.window-menu.group.create": "Create group",
    "bar.window-menu.group.move-out": "Move out of group",
    "bar.window-menu.group.ungroup": "Ungroup",

    // Widget names (bar + settings panel)
    "widget.bluetooth.name": "Bluetooth",
    "widget.bluetooth.sub.active": "Active",
    "widget.bluetooth.sub.inactive": "Inactive",
    "widget.dark-mode.name": "Appearance",
    "widget.dark-mode.sub.dark": "Dark",
    "widget.dark-mode.sub.light": "Light",
    "widget.focus.name": "Do not disturb",
    "widget.cpu-memory.name": "CPU & memory",
    "widget.cpu-memory.cpu": "CPU",
    "widget.cpu-memory.ram": "RAM",

    // Ethernet bar popover rows
    "widget.ethernet.row.interface": "Interface",
    "widget.ethernet.row.status": "Status",
    "widget.ethernet.row.speed": "Speed",

    // Wi-Fi bar popover rows
    "widget.wifi.row.network": "Network",
    "widget.wifi.row.status": "Status",
    "widget.wifi.row.disabled": "Disabled",

    // Activity Island — live screen capture
    "island.recording.title": "Recording screen",

    // Activity Island — battery-critical alert
    "island.battery.critical": "Battery critically low",
    "island.battery.hint": "Connect the charger",

    // Activity Island — the built-in Assistant
    "island.agent.title": "Assistant",
    "island.agent.status.idle": "Assistant",
    "island.agent.status.thinking": "Thinking…",
    "island.agent.status.acting": "Working…",
    "island.agent.placeholder": "Ask or tell me to do something…",
    "island.agent.reset": "New conversation",
    "island.agent.tokens": "%s tokens",
    "island.agent.tokens-cached": "%d% cached",
    "island.agent.empty": "Choose a provider and add your API key to start using the assistant.",
    "island.agent.open-settings": "Open Settings → AI",
    // Abnormal ends. A turn NEVER finishes without either a reply or one of these.
    "island.agent.error.died": "The assistant stopped unexpectedly. Send your message again to retry.",
    "island.agent.error.empty": "The assistant ended the turn without replying.",

    // Battery widget
    "widget.battery.label.unavailable": "Battery not available",
    "widget.battery.name": "Battery",
    "widget.battery.state.charged": "Charged",
    "widget.battery.state.charging": "Charging",
    "widget.battery.state.discharging": "Discharging",
    "widget.clipboard.name": "Clipboard",
    "widget.clipboard.empty": "No clipboard history",
    "widget.clipboard.image": "Image",
    "widget.clipboard.delete": "Delete",
    "widget.clipboard.clear": "Clear history",
    "widget.clipboard.clear.confirm": "Clear everything",
    "widget.clipboard.clear.cancel": "Cancel",
    "widget.screenshot.name": "Screenshot",
    "widget.screenshot.mode.area": "Area",
    "widget.screenshot.mode.screen": "Screen",
    "widget.screenshot.mode.window": "Window",
    "widget.screenshot.action.copy": "Copy",
    "widget.screenshot.action.save": "Save",
    "widget.screenshot.saved": "Screenshot saved",
    "widget.screenrecord.name": "Screen recording",
    "widget.screenrecord.mode.screen": "Full screen",
    "widget.screenrecord.mode.region": "Region",
    "widget.screenrecord.audio": "Include audio",
    "widget.screenrecord.start": "Start recording",
    "widget.screenrecord.recording": "Recording…",
    "widget.screenrecord.stop": "Stop recording",
    "widget.screenrecord.saved": "Recording saved",
    "widget.screenrecord.no-audio": "No audio source was available, so the recording is silent.",
    "widget.screenrecord.settings.audio-group": "Audio",
    "widget.screenrecord.settings.source": "Source",
    "widget.screenrecord.settings.source.desc": "What the Audio switch records.",
    "widget.screenrecord.settings.source.system": "System audio",
    "widget.screenrecord.settings.source.mic": "Microphone",
    "widget.screenrecord.settings.video-group": "Video",
    "widget.screenrecord.settings.quality": "Quality",
    "widget.screenrecord.settings.quality.desc": "Higher quality means larger files.",
    "widget.screenrecord.settings.quality.low": "Low",
    "widget.screenrecord.settings.quality.balanced": "Balanced",
    "widget.screenrecord.settings.quality.high": "High",
    "widget.screenrecord.settings.framerate": "Frame rate",
    "widget.screenrecord.settings.framerate.desc": "Automatic follows the screen and writes the smallest file.",
    "widget.screenrecord.settings.framerate.auto": "Automatic",
    "widget.screenrecord.settings.hardware": "Hardware encoding",
    "widget.screenrecord.settings.hardware.desc": "Encode on the GPU. Applies to MP4 and MKV; WebM always encodes in software.",
    "widget.screenrecord.settings.file-group": "File",
    "widget.screenrecord.settings.format": "Format",
    "widget.screenrecord.settings.format.desc": "MKV survives a crash mid-recording; MP4 plays anywhere.",
    "widget.screenrecord.settings.save-to": "Save to",
    "widget.screenrecord.settings.choose-folder": "Choose…",
    "widget.screenrecord.settings.dialog.folder": "Select a folder for recordings",
    "widget.brightness.name": "Brightness",
    "widget.night-light.name": "Night light",
    "widget.night-light.sub.off": "Off",

    // Control Center — Ethernet toggle
    "cc.ethernet.name": "Ethernet",
    "cc.ethernet.sub.no-cable": "No cable",
    "cc.ethernet.sub.disconnected": "Disconnected",
    "cc.ethernet.sub.connected": "Connected",

    // Control Center — Wi-Fi toggle
    "cc.wifi.name": "Wi-Fi",
    "cc.wifi.sub.off": "Off",
    "cc.wifi.sub.connected": "Connected",
    "cc.wifi.sub.disconnected": "Not connected",

    // Control Center — Focus/DnD toggle
    "cc.focus.name": "Focus",
    "cc.focus.title.on": "Focus",
    "cc.focus.title.off": "Focus",
    "cc.focus.sub.on": "Do not disturb",

    // Control Center — Island grid
    "cc.menu.size.small": "Small",
    "cc.grid.done": "Done",
    "cc.grid.edit": "Edit",
    "cc.menu.size.medium": "Medium",
    "cc.menu.size.large": "Large",
    "cc.menu.details": "Show details",
    "cc.menu.remove": "Remove",
    "cc.menu.size-full": "Grid is full",

    // Control Center — Volume widget
    "cc.volume.name": "Volume",

    // Control Center — Media widget
    "cc.media.name": "Media",
    "cc.media.no-media": "No media",
    "cc.media.source": "Audio source",
    "cc.media.source.auto": "Automatic",

    // Notification Center
    "nc.time.now": "now",
    "nc.empty": "No notifications",
    "nc.clear-all": "Clear notifications",

    // Gaming
    "settings.gaming.title":                     "Gaming",
    "settings.gaming.group.wallpaper":                       "Wallpaper",
    "settings.gaming.group.performance":                     "Performance",
    "settings.gaming.mode.artwork":                          "Game art",
    "settings.gaming.mode.custom":                           "Custom",
    "settings.gaming.mode.none":                             "None",
    "settings.gaming.wallpaper-mode":              "Wallpaper",
    "settings.gaming.wallpaper-mode.desc":               "What to display when a game launches",
    "settings.gaming.custom-wallpaper":            "Image",
    "settings.gaming.custom-wallpaper.desc":             "Wallpaper shown for all games",
    "settings.gaming.transition":                        "Transition",
    "settings.gaming.transition.desc":                   "Transition when switching to game wallpaper",
    "settings.gaming.performance-profile":         "High-performance profile",
    "settings.gaming.performance-profile.desc":          "Activate performance power profile automatically when a game launches",
    "settings.gaming.dialog.wallpaper":    "Select wallpaper",
    // XDG portal consent prompts (surfaces/consent) — fallback button labels only;
    // the frontend normally supplies its own, already translated.
    "consent.allow": "Allow",
    "consent.deny": "Deny",
}
