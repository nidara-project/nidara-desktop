...
function buildAppPickerPage(onPick: (cmd: string) => void, existingCommands: string[]): Gtk.Widget {
    ...
    appService.getAllApps().forEach(app => {
        ...
        const trailing = added
            ...
            : new Gtk.Image({
                gicon: uiIcon("nd-value-increase"), pixel_size: 16,
    ...
    rowBox.append(new Gtk.Image({ gicon: uiIcon("nd-utilities-terminal"), pixel_size: 18, valign: Gtk.Align.CENTER, opacity: 0.6, css_classes: ["nd-icon"] }))
...
export default function AutostartPage(nav: SettingsNav) {
    ...
    const buildEntryRow = (entry: AutostartEntry, idx: number): Gtk.ListBoxRow => {
        ...
        const leadingIcon = app
            ...
            : new Gtk.Image({ gicon: uiIcon("nd-utilities-terminal"), pixel_size: 18, width_request: 32, css_classes: ["nd-icon"] })
        ...
        const deleteBtn = new Gtk.Button({
            child: new Gtk.Image({ gicon: uiIcon("nd-user-trash"), pixel_size: 16, css_classes: ["nd-icon"] }),
    ...
    const buildAddNavRow = (): Gtk.ListBoxRow => {
        const plusIcon = new Gtk.Image({
            gicon: uiIcon("nd-value-increase"), pixel_size: 18, width_request: 32,
    ...
    const refresh = () => {
        ...
        if (entries.length === 0) {
            listBox.append(NidaraEmptyRow(t("settings.autostart.empty")))
        } else {
