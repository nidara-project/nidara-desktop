...
function buildBrightnessIcon(): Gtk.Widget {
    return new Gtk.Image({
        gicon: uiIcon("nd-display-brightness"), pixel_size: 28,
...
function buildVertical(): Gtk.Widget {
    ...
    const tile = makeVerticalFillTile(uiIcon("nd-display-brightness"), {
        value: _cachedPct,
...
function buildHorizontal(): Gtk.Widget {
    ...
    const box = makeHSliderTile({
        ...
        low:  { icon: uiIcon("nd-display-brightness-low"), size: 14, opacity: 0.5 },
        high: { icon: uiIcon("nd-display-brightness") },
        getValue: () => _cachedPct,
        onChange: (v) => {
...
function buildBarExpanded(_onClose: () => void): Gtk.Widget {
    ...
    row.append(new Gtk.Image({ gicon: uiIcon("nd-display-brightness-low"), pixel_size: 14, opacity: 0.5, css_classes: ["nd-icon"] }))
    row.append(slider)
