...
function buildContent(size: WidgetSize, budget: ContentBudget): Gtk.Widget {
    ...
    // Space the two metrics one host pitch apart so each ring lands on its grid-cell
    // centre — i.e. exactly where a 1×1 widget's icon sits — instead of bunched in the
    // middle of the tile. The ring is the widest part of each metric column (caption
    // "CPU"/"RAM" is narrower), so column width ≈ ring and a spacing of (pitch − ring)
    // puts the ring centres one pitch apart. The CenterBox keeps the pair centred in
    // the tile, which (being symmetric) lands them on both cell centres. The pitch
    // comes from the host — this widget used to rebuild it from UNIT + GAP.
    const inner = new Gtk.Box({
        spacing: budget.pitch - ring,
        halign: Gtk.Align.CENTER,
        valign: Gtk.Align.CENTER,
    })
