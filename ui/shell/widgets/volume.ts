...
function buildCCContent(size: WidgetSize): Gtk.Widget {
    ...
    if (size === WidgetSize.TALL) {
        ...
        return makeVerticalFillTile(
            () => { const s = speaker(); return s ? AudioSvc.targetVolumeIcon(s) : uiIcon("nd-audio-volume-muted") },
    ...
    return makeHSliderTile({
        low:  { icon: uiIcon("nd-audio-volume-low") },
        high: { icon: uiIcon("nd-audio-volume-high") },
...
function buildBarExpanded(_onClose: () => void): Gtk.Widget {
    ...
    const muteImg = new Gtk.Image({ gicon: (speaker as any)?.mute ? uiIcon("nd-audio-volume-muted") : uiIcon("nd-audio-volume-high"), pixel_size: 16, css_classes: ["nd-icon"] })
    ...
    muteBtn.connect("clicked", () => {
        ...
        muteImg.gicon = (speaker as any)?.mute ? uiIcon("nd-audio-volume-muted") : uiIcon("nd-audio-volume-high")
    })
...
function buildSpeakerRow(ep: any, isDefault: boolean): Gtk.ListBoxRow {
    ...
    sliderRow.append(new Gtk.Image({ gicon: uiIcon("nd-audio-volume-low"), pixel_size: 14, opacity: 0.5, css_classes: ["nd-icon"] }))
    sliderRow.append(scale)
    sliderRow.append(new Gtk.Image({ gicon: uiIcon("nd-audio-volume-high"), pixel_size: 14, opacity: 0.5, css_classes: ["nd-icon"] }))
