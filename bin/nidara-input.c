// ─────────────────────────────────────────────────────────────────────────────
// nidara-input — synthetic POINTER for the computer-use layer (Phase 2b-ii).
...
//
//   nidara-input move       <x> <y> <w> <h>
//   nidara-input click      <x> <y> <w> <h>          # left button
//   nidara-input rightclick <x> <y> <w> <h>          # right button
//   nidara-input scroll     <x> <y> <w> <h> <dx> <dy> # wheel notches at (x,y)
//   nidara-input drag       <x> <y> <w> <h> <x2> <y2> # press at (x,y), glide to (x2,y2), release
//
...
// geometry mapping live in the nidara-click GJS wrapper. Single-output for now
...
//   cc nidara-input.c <generated>.c $(pkg-config --cflags --libs wayland-client)
// ─────────────────────────────────────────────────────────────────────────────
...
int main(int argc, char **argv) {
    ...
    if ((!is_move && !is_click && !is_right && !is_scroll && !is_drag) ||
        ...
        fprintf(stderr,
            "usage: nidara-input move|click|rightclick <x> <y> <w> <h>\n"
            "       nidara-input scroll <x> <y> <w> <h> <dx> <dy>\n"
            "       nidara-input drag   <x> <y> <w> <h> <x2> <y2>\n");
        return 2;
    }
    ...
    if (!dpy) { fprintf(stderr, "nidara-input: cannot connect to Wayland display\n"); return 1; }
    ...
    if (!mgr) {
        fprintf(stderr, "nidara-input: compositor does not expose zwlr_virtual_pointer_manager_v1\n");
        return 1;
    }
